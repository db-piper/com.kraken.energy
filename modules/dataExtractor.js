...
module.exports = class dataExtractor {
   ...
   */
  extractDeviceDefinitions(rawPairingData, statusTranslations) {
    ...
    const validStatusCodes = Object.keys(statusTranslations);
   ...
   */
  extractAllDeviceDispatches(rawPayload, deviceStates) {
    const dispatchMap = {};
    for (const { id, currentState, currentStateTitle } of deviceStates) {
      ...
      if (!this._dispatchable_device_status.includes(currentState)) {
        continue;
      }
